...
function EmployeeApp() {
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0f] text-white flex flex-col relative">
            ...
            <div className="flex-1 overflow-y-auto p-4 pb-24">
                ...
                {view === 'service' && (
                    <div className="max-w-md mx-auto space-y-3">
                        <div className="bg-amber-900/20 border border-amber-500/30 p-3 rounded-xl flex justify-between items-center mb-4 backdrop-blur-sm">
                            <div><h2 className="text-xl font-bold tracking-wider text-amber-100">{entryData.plate}</h2><p className="text-xs text-amber-300">{entryData.km} χλμ {currentServiceId ? '(ΕΝΗΜΕΡΩΣΗ)' : '(ΝΕΑ)'}</p></div>
                            <button onClick={() => { setView('entry'); fetchPendingServices(); }} className="text-xs bg-[#12121a] hover:bg-[#1a1a25] px-3 py-1 rounded border border-amber-900/30 text-gray-300">ΠΙΣΩ</button>
                        </div>
                        {SECTIONS.map(sec => (
                            <div key={sec.id} className="bg-[#12121a]/80 backdrop-blur-lg border border-amber-900/20 rounded-xl overflow-hidden shadow-xl shadow-black/20">
                                <button onClick={() => setExpandedSection(expandedSection === sec.id ? null : sec.id)} className={`w-full p-4 flex justify-between items-center font-bold transition-colors ${expandedSection === sec.id ? 'bg-amber-900/20 text-amber-100' : 'text-gray-300 hover:bg-amber-900/10'}`}>
                                    <span className="flex items-center gap-2">{sec.icon} {sec.title}</span>
                                    {expandedSection === sec.id ? <ChevronUp size={20} className="text-amber-400" /> : <ChevronDown size={20} />}
                                </button>
                                <AnimatePresence>
                                    {expandedSection === sec.id && (
                                        <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="border-t border-amber-900/20 bg-[#0a0a0f]">
                                            <div className="p-4 space-y-6">
                                                {sec.items.map(item => (
                                                    <div key={item} className="space-y-2 border-b border-amber-900/20 pb-3 last:border-0">
                                                        <div className="font-medium text-sm text-gray-300">{item}</div>
                                                        {sec.type === 'choice' && (<div className="flex flex-wrap gap-2">{sec.options.map(opt => { const active = selections[item] === opt; let style = 'bg-[#12121a] border-amber-900/30 text-gray-500 hover:bg-amber-900/10'; if (active) { if (opt.includes('ΑΛΛΑΓΗ')) style = 'bg-red-600 border-red-600 text-white shadow-lg shadow-red-900/20'; else if (opt.includes('ΕΛΕΓΧΟΣ')) style = 'bg-green-600 border-green-600 text-white shadow-lg shadow-green-900/20'; else style = 'bg-amber-600 border-amber-600 text-white shadow-lg shadow-amber-900/20'; } return (<button key={opt} onClick={() => toggleSelection(item, opt)} className={`flex-1 py-3 text-[10px] sm:text-xs font-bold border rounded-lg transition-all active:scale-95 ${style}`}>{opt}</button>) })}</div>)}
                                                        {sec.type === 'check' && (<button onClick={() => toggleExtra(item)} className={`w-full py-3 flex items-center justify-center gap-2 border rounded-lg font-bold transition-all active:scale-95 ${extras[item] ? 'bg-amber-600 border-amber-600 text-white shadow-lg' : 'border-amber-900/30 text-gray-500 hover:bg-amber-900/10'}`}>{extras[item] && <CheckCircle size={16} />} {extras[item] ? 'ΕΓΙΝΕ' : 'ΕΠΙΛΟΓΗ'}</button>)}
                                                    </div>
                                                ))}
                                                {sec.extras && (<div className="pt-2 border-t border-amber-900/20">{sec.extras.map(ex => (<button key={ex} onClick={() => toggleExtra(ex)} className={`w-full py-3 flex items-center justify-between px-4 rounded-lg border mb-2 transition-all active:scale-95 ${extras[ex] ? 'bg-amber-900/30 border-amber-500 text-amber-300' : 'bg-[#12121a] border-amber-900/20 text-gray-500 hover:bg-amber-900/10'}`}><span className="text-xs font-bold">{ex}</span>{extras[ex] ? <CheckCircle size={18} /> : <div className="w-4 h-4 border border-amber-900/30 rounded" />}</button>))}</div>)}
                                                <div><label className="text-[10px] font-bold text-gray-500 mb-1 block">ΣΧΟΛΙΑ</label><textarea className="w-full bg-[#0a0a0f] border border-amber-900/30 rounded-lg p-3 text-sm outline-none focus:border-amber-500/50 placeholder-gray-700" rows="2" placeholder="Γράψε σημειώσεις..." value={comments[sec.id] || ''} onChange={e => setComments({ ...comments, [sec.id]: e.target.value })} /></div>
                                            </div>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
            ...
            {view === 'service' && (
                <div className="fixed bottom-0 left-0 right-0 bg-[#0d0d14]/95 backdrop-blur-2xl border-t border-amber-900/20 p-4 flex gap-3 z-30 shadow-[0_-5px_30px_rgba(0,0,0,0.6)]">
                    <button onClick={() => handleSave('Temp')} disabled={loading} className="flex-1 bg-[#12121a] hover:bg-[#1a1a25] text-gray-300 font-bold py-3 rounded-xl border border-amber-900/30 flex justify-center items-center gap-2 active:scale-95 transition-transform"><Clock size={18} className="text-amber-400" /> ΠΡΟΣΩΡΙΝΗ</button>
                    <button onClick={() => handleSave('Completed')} disabled={loading} className="flex-[2] bg-gradient-to-r from-amber-600 to-yellow-500 hover:from-amber-500 hover:to-yellow-400 text-black font-bold py-3 rounded-xl shadow-lg shadow-amber-900/30 flex justify-center items-center gap-2 active:scale-95 transition-transform">{loading ? <Loader2 className="animate-spin" /> : <><Save size={18} /> ΟΛΟΚΛΗΡΩΣΗ</>}</button>
                </div>
            )}
        </div>
    );
