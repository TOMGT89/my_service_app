                                                        ...
                                                        <div key={i} className={`space-y-1 ${isPrinting ? 'col-span-1 border-l-2 border-slate-100 pl-2' : ''}`}>
                                                            ...
                                                            <div className={`grid gap-1 ${isPrinting ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2 shadow-sm'}`}>
                                                                {cat.items.map((item, j) => (
                                                                    <div key={j} className={`text-slate-700 flex justify-between items-center bg-slate-50 border border-slate-100 transition-all ${isPrinting ? 'text-[9px] p-1 rounded-sm shadow-none' : 'text-sm p-3 rounded-lg'}`}>
                                                                        <span className="font-medium truncate pr-2">{item.name}</span>
                                                                        <span className={`font-bold rounded uppercase tracking-tighter ${isPrinting ? 'text-[7px] px-1' : 'text-[9px] px-2 py-0.5'} ${item.action === 'ΑΛΛΑΓΗ' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                                                                            {item.action}
                                                                        </span>
                                                                    </div>
                                                                ))}
                ...
                <div className={`bg-[#1e293b] border-t border-slate-200 flex flex-wrap justify-center mt-auto ${isPrinting ? 'p-2 gap-4' : 'p-8 gap-6 print:hidden'}`}>
                    {settings?.phones?.map(phone => (
                        <a key={phone} href={`tel:${phone}`} className={`flex items-center gap-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all border border-white/10 text-white no-underline ${isPrinting ? 'px-4 py-2' : 'px-6 py-3'}`}>
                            <div className="bg-green-500/20 p-2 rounded-full text-green-400"><Phone size={isPrinting ? 14 : 20} /></div>
                    ...
                    {settings?.website && (
                        <a href={settings.website} target="_blank" rel="noreferrer" className={`flex items-center gap-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all border border-white/10 text-white no-underline ${isPrinting ? 'px-4 py-2' : 'px-6 py-3'}`}>
                            <div className="bg-blue-500/20 p-2 rounded-full text-blue-400"><Globe size={isPrinting ? 14 : 20} /></div>
