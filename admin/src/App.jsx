                    ...
                    {loading ? <p className="text-center text-slate-500">Φόρτωση...</p> : history.length === 0 ? (
                        ...
                        history.map((record) => (
                            <div key={record._id} className="bg-slate-800 border border-slate-700 rounded-xl p-5 hover:border-slate-600 transition-colors shadow-sm">
                                ...
                                <div className="space-y-3 mb-4">{record.servicesPerformed.map((cat, i) => (<div key={i} className="bg-slate-900/50 rounded-lg p-3 border border-slate-700/30"><h4 className="text-[10px] font-bold text-slate-500 mb-2 uppercase tracking-wider">{cat.categoryTitle}</h4><div className="space-y-1">{cat.items.map((item, j) => (<div key={j} className="flex items-center gap-2 text-sm text-slate-300"><CheckCircle size={14} className="text-green-500 shrink-0" /><span className="font-medium text-white">{item.name}</span><span className="text-slate-500 text-xs px-2 py-0.5 bg-slate-800 rounded border border-slate-700 ml-auto">{item.action}</span></div>))}</div></div>))}</div>
...
const MiniERPTab = ({ theme, user }) => {
    ...
    const fetchData = async () => {
        ...
        setUsers(await (await fetch(`${API_URL}/api/users`, { headers })).json()); // NEO: Fetch users
    };
    ...
    return (
        <div className="space-y-8">
            <div className={`flex flex-col md:flex-row justify-between items-center ${theme.card} p-4 rounded-xl border ${theme.border}`}>
                <h2 className="text-xl font-bold text-white flex gap-2"><Wallet className="text-blue-400" /> Οικονομικά</h2>
                <div className="flex gap-2">
                    <select value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} className={`${theme.input} text-white border ${theme.border} rounded px-2 py-1 text-sm`}>{fullMonths.map((m, i) => <option key={i} value={i}>{m}</option>)}</select>
                    <select value={selectedYear} onChange={e => setSelectedYear(e.target.value)} className={`${theme.input} text-white border ${theme.border} rounded px-2 py-1 text-sm`}><option value={2025}>2025</option><option value={2026}>2026</option></select>
                </div>
            </div>

            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                <div className={`${theme.card} p-4 rounded-xl border ${theme.border} text-center shadow-lg`}><p className="text-[10px] text-slate-400 font-bold uppercase">Τζιρος</p><p className="text-xl font-bold text-green-400">+{totalRevenue} €</p></div>
                <div className={`${theme.card} p-4 rounded-xl border ${theme.border} text-center shadow-lg`}><p className="text-[10px] text-slate-400 font-bold uppercase">Ανταλ/κα</p><p className="text-xl font-bold text-orange-400">-{totalPartsCost} €</p></div>
                <div className={`${theme.card} p-4 rounded-xl border ${theme.border} text-center shadow-lg`}><p className="text-[10px] text-slate-400 font-bold uppercase">Μισθοδοσια</p><p className="text-xl font-bold text-red-400">-{totalSalaries} €</p></div>
                <div className={`${theme.card} p-4 rounded-xl border ${theme.border} text-center shadow-lg`}>
                ...
                <div className={`col-span-2 lg:col-span-1 ${theme.input} p-4 rounded-xl border ${theme.border} text-center shadow-lg bg-opacity-50`}><p className={`text-[10px] ${theme.accent} font-bold uppercase`}>Καθαρο Κερδος</p><p className={`text-2xl font-bold ${netProfit >= 0 ? 'text-white' : 'text-red-500'}`}>{netProfit} €</p></div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className={`lg:col-span-2 ${theme.card} p-6 rounded-xl border ${theme.border} overflow-visible relative shadow-lg`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                <div className={`${theme.card} rounded-xl border ${theme.border} p-4 shadow-lg`}>
                    ...
                    <form onSubmit={handleAddRecurring} className="flex gap-2 mb-4"><input placeholder="π.χ. Ενοίκιο" className={`${theme.input} border ${theme.border} text-white px-2 rounded flex-1 text-sm`} value={newRec.title} onChange={e => setNewRec({ ...newRec, title: e.target.value })} /><input type="number" placeholder="€" className={`${theme.input} border ${theme.border} text-white px-2 rounded w-20 text-sm`} value={newRec.amount} onChange={e => setNewRec({ ...newRec, amount: e.target.value })} /><button className={`${theme.button} text-white px-3 rounded font-bold hover:bg-blue-500 text-sm`}>OK</button></form>
                    <div className="space-y-2">{recurring.map(r => (<div key={r._id} className={`flex justify-between items-center text-sm ${theme.input} p-2 rounded`}><span className="text-slate-300 break-all">{r.title}</span><div className="flex gap-2 items-center">{editRecId === r._id ? (<div className="flex items-center gap-1"><input className={`w-16 ${theme.card} text-white border ${theme.border} rounded px-1`} value={editAmount} onChange={e => setEditAmount(e.target.value)} autoFocus /><button onClick={() => handleUpdateRecurring(r._id)} className="text-green-400"><CheckCircle size={14} /></button><button onClick={() => setEditRecId(null)} className="text-slate-400"><X size={14} /></button></div>) : (<div className="flex items-center gap-2"><span className="font-bold text-white">{r.amount} €</span><button onClick={() => { setEditRecId(r._id); setEditAmount(r.amount); }} className="text-yellow-500 hover:text-yellow-400"><Pencil size={14} /></button><button onClick={() => handleDeleteRecurring(r._id)} className={`${deleteConfirmId === r._id ? 'text-red-500 font-bold bg-white/10 px-2 rounded' : 'text-red-400 hover:text-red-300'}`}>{deleteConfirmId === r._id ? 'Σίγουρα;' : <X size={14} />}</button></div>)}</div></div>))}</div>
                </div>
                <div className={`${theme.card} rounded-xl border ${theme.border} overflow-hidden shadow-lg`}>
                    <div className={`p-3 ${theme.sidebar} border-b ${theme.border} font-bold text-slate-400 text-sm flex justify-between items-center`}><span>Έξοδα Μήνα ({fullMonths[selectedMonth]})</span><div className="flex gap-1"><input placeholder="Νέο έξοδο.." className={`w-24 ${theme.input} border ${theme.border} text-white px-1 text-xs rounded`} value={newMonthly.title} onChange={e => setNewMonthly({ ...newMonthly, title: e.target.value })} /><input type="number" placeholder="€" className={`w-12 ${theme.input} border ${theme.border} text-white px-1 text-xs rounded`} value={newMonthly.amount} onChange={e => setNewMonthly({ ...newMonthly, amount: e.target.value })} /><button onClick={handleAddExpense} className="text-green-400 hover:text-green-300"><Plus size={16} /></button></div></div>
                    <div className="max-h-60 overflow-y-auto">{filteredExpenses.map(ex => (<div key={ex._id} className={`p-3 border-b ${theme.border} flex justify-between items-center hover:bg-white/5 text-sm`}><div><p className="font-bold text-white break-words max-w-[200px]">{ex.title}</p></div><div className="flex items-center gap-2">{editExpId === ex._id ? (<div className="flex items-center gap-1"><input className={`w-16 ${theme.input} text-white border ${theme.border} rounded px-1`} value={editAmount} onChange={e => setEditAmount(e.target.value)} autoFocus /><button onClick={() => handleUpdateExpense(ex._id)} className="text-green-400"><CheckCircle size={14} /></button><button onClick={() => setEditExpId(null)} className="text-slate-400"><X size={14} /></button></div>) : (<div className="flex items-center gap-2"><span className="text-red-400 font-bold">-{ex.amount} €</span><button onClick={() => { setEditExpId(ex._id); setEditAmount(ex.amount); }} className="text-yellow-500 hover:text-yellow-400"><Pencil size={14} /></button><button onClick={() => handleDeleteExpense(ex._id)} className={`${deleteConfirmId === ex._id ? 'text-red-500 font-bold bg-white/10 px-2 rounded' : 'text-red-400 hover:text-red-300'}`}>{deleteConfirmId === ex._id ? 'Σίγουρα;' : <X size={14} />}</button></div>)}</div></div>))}
...
const GarageTab = ({ vehicles, refreshVehicles, theme, user }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4"><h2 className="text-2xl font-bold text-white">🚖 Διαχείριση Στόλου</h2><div className="flex gap-3 w-full md:w-auto"><div className="relative flex-1 md:w-64"><Search className={`absolute left-3 top-1/2 -translate-y-1/2 ${theme.text}`} size={18} /><input type="text" placeholder="Αναζήτηση..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={`w-full ${theme.input} border ${theme.border} text-white pl-10 pr-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none uppercase`} /></div><button onClick={() => setShowForm(!showForm)} className={`${theme.button} text-white px-4 py-2 rounded-lg flex gap-2 items-center`}><Plus size={18} /> <span className="hidden md:inline">Νέα Εισαγωγή</span></button></div></div>
            <AnimatePresence>{showForm && (<motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} onSubmit={handleAdd} className={`${theme.card} p-4 rounded-xl border ${theme.border} grid grid-cols-1 md:grid-cols-2 gap-4 overflow-hidden`}><input placeholder="Πινακίδα (ΑΒΓ-1234)" value={newCar.plateNumber} onChange={e => setNewCar({ ...newCar, plateNumber: e.target.value.toUpperCase() })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg w-full font-bold uppercase tracking-wider`} /><div className="grid grid-cols-2 gap-2"><input placeholder="Μάρκα" value={newCar.brand} onChange={e => setNewCar({ ...newCar, brand: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg`} /><input placeholder="Μοντέλο" value={newCar.model} onChange={e => setNewCar({ ...newCar, model: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg`} /></div><input placeholder="Ονοματεπώνυμο Πελάτη" value={newCar.ownerName} onChange={e => setNewCar({ ...newCar, ownerName: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg`} /><input placeholder="Τηλέφωνο Επικοινωνίας" value={newCar.ownerPhone} onChange={e => setNewCar({ ...newCar, ownerPhone: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg`} /><button type="submit" className="col-span-1 md:col-span-2 bg-green-600 text-white p-3 rounded-lg font-bold hover:bg-green-500 shadow-lg mt-2">Αποθήκευση Οχήματος</button></motion.form>)}</AnimatePresence>
            ...
            <div className="flex flex-col gap-4">
                {filteredVehicles.length === 0 ? (<p className="text-slate-500 text-center py-10">Δεν βρέθηκαν οχήματα.</p>) : (
                    filteredVehicles.map((car) => {
                        ...
                        const jobDescription = lastService?.servicesPerformed?.map(cat => cat.items.map(i => i.name).join(', ')).join(' + ') || 'Καμία καταγραφή';

                        return (
                            <motion.div key={car._id} layout className={`${theme.card} border ${theme.border} p-0 rounded-xl flex flex-col md:flex-row overflow-hidden hover:border-slate-500 transition-colors`}>
                                {/* 1. CAR INFO (LEFT) */}
                                <div className={`p-4 ${theme.sidebar} md:w-64 flex flex-col justify-center border-b md:border-b-0 md:border-r ${theme.border}`}>
                                    ...
                                    <div className={`mt-3 pt-3 border-t ${theme.border} flex flex-col gap-1 text-xs text-slate-500`}>
                                ...
                                <div className="p-4 flex-1 flex flex-col justify-center">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <div className="flex gap-2 items-center">
                                            ...
                                            {lastService && <span className={`text-xs text-slate-400 ${theme.input} px-2 py-1 rounded border ${theme.border}`}>{new Date(lastService.date).toLocaleDateString('el-GR')}</span>}
                                        </div>
                                    </div>
                                ...
                                <div className={`p-4 ${theme.sidebar} md:w-80 flex flex-col justify-between gap-3 border-t md:border-t-0 md:border-l ${theme.border}`}>
                                    {/* Price Inputs */}
                                    {lastService ? (
                                        editingPriceServiceId === lastService._id ? (
                                            <div className={`flex items-center gap-2 ${theme.input} p-2 rounded border ${theme.border} animate-in fade-in`}>
                                                <div><label className="text-[10px] text-green-500 block">Έσοδο</label><input type="number" className={`w-16 ${theme.card} text-white text-right px-1 rounded text-sm outline-none border ${theme.border}`} value={quickPrice} onChange={e => setQuickPrice(e.target.value)} /></div>
                                                <div><label className="text-[10px] text-red-500 block">Κόστος</label><input type="number" className={`w-16 ${theme.card} text-white text-right px-1 rounded text-sm outline-none border ${theme.border}`} value={quickCost} onChange={e => setQuickCost(e.target.value)} /></div>
                                            ...
                                            <div className={`flex justify-between items-center ${theme.input} px-3 py-2 rounded-lg border ${theme.border}`}>
                                        ...
                                        <div className={`text-center text-xs text-slate-600 py-3 ${theme.input} rounded-lg border ${theme.border}`}>Χωρίς καταγραφή τιμής</div>
                                    )}

                                    {/* Buttons */}
                                    <div className="flex gap-3 justify-end mt-auto">
                                        <button onClick={() => setSelectedCar(car)} className={`flex-1 ${theme.button} text-white text-xs font-bold py-2 px-3 rounded flex items-center justify-center gap-2 transition-colors`} title="Προβολή Ιστορικού"><Clock size={16} /> Ιστορικό</button>

                                        {/* QR BUTTON */}
                                        <button onClick={() => setQrVehicle(car)} className={`p-3 ${theme.input} hover:bg-slate-700 text-slate-400 hover:text-white rounded transition-colors`} title="Εκτύπωση QR"><QrCode size={18} /></button>

                                        <button onClick={() => setEditingCar(car)} className={`p-3 ${theme.input} hover:bg-slate-700 text-slate-400 hover:text-yellow-400 rounded transition-colors`} title="Επεξεργασία"><Pencil size={18} /></button>
...
const UsersTab = ({ theme, user }) => {
    ...
    const fetchUsers = async () => {
        const res = await fetch(`${API_URL}/api/users`, { headers: { 'x-user-id': user._id } }); setUsers(await res.json());
    };
    useEffect(() => { fetchUsers(); }, []);

    const handleSubmit = async (e) => {
        e.preventDefault();
        const headers = { 'Content-Type': 'application/json', 'x-user-id': user._id };
        if (editingId) { await fetch(`${API_URL}/api/users/${editingId}`, { method: 'PUT', headers, body: JSON.stringify(formData) }); setEditingId(null); }
        else { await fetch(`${API_URL}/api/users`, { method: 'POST', headers, body: JSON.stringify(formData) }); }
        setFormData({ username: '', password: '', salary: '', insurance: '' }); fetchUsers();
    };
    ...
    const handleDelete = async (id) => { if (!confirm('Διαγραφή;')) return; await fetch(`${API_URL}/api/users/${id}`, { method: 'DELETE', headers: { 'x-user-id': user._id } }); fetchUsers(); };
    ...
    return (
        <div className="space-y-6">
            ...
            <form onSubmit={handleSubmit} className={`${theme.card} p-4 rounded-xl border ${theme.border}`}>
                <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
                    <input placeholder="Username" value={formData.username} onChange={e => setFormData({ ...formData, username: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-2 rounded`} />
                    <input placeholder="Password" value={formData.password} onChange={e => setFormData({ ...formData, password: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-2 rounded`} />
                    <input type="number" placeholder="Καθαρός Μισθός (€)" value={formData.salary} onChange={e => setFormData({ ...formData, salary: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-2 rounded`} />
                    <input type="number" placeholder="Κόστος Ασφάλισης (€)" value={formData.insurance} onChange={e => setFormData({ ...formData, insurance: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-2 rounded`} />
                    <div className="flex gap-2">
...
const SettingsTab = ({ user, setUser }) => {
    ...
    const theme = THEMES[user.theme || 'default']; // For styling THIS component based on current user theme
    ...
    return (
        <div className="space-y-8 max-w-2xl"><h2 className="text-2xl font-bold text-white">⚙️ Ρυθμίσεις Καταστήματος</h2>
            ...
            {/* THEME SELECTOR */}
            <div className={`${theme.card} p-6 rounded-xl border ${theme.border} space-y-4`}>
                <label className={`block ${theme.text} mb-2 font-bold`}>Επιλογή Θέματος</label>
        ...
        <div className="space-y-8">
            ...
            <div className={`${theme.card} p-6 rounded-xl border ${theme.border} space-y-4`}>
                ...
                <form onSubmit={handleCreateShop} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input placeholder="Όνομα Συνεργείου" value={newShop.name} onChange={e => setNewShop({ ...newShop, name: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg`} required />
                    <input placeholder="Email (Username)" value={newShop.email} onChange={e => setNewShop({ ...newShop, email: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg`} required />
                    <input placeholder="Κωδικός Πρόσβασης" value={newShop.password} onChange={e => setNewShop({ ...newShop, password: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg`} required />
                    <select value={newShop.plan} onChange={e => setNewShop({ ...newShop, plan: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg`}>
                        <option value="Basic">Basic Plan</option>
                        <option value="Pro">Pro Plan</option>
                        <option value="Enterprise">Enterprise</option>
                    </select>
                    <select value={newShop.theme} onChange={e => setNewShop({ ...newShop, theme: e.target.value })} className={`${theme.input} border ${theme.border} text-white p-3 rounded-lg`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {shops.map(shop => (
                    <div key={shop._id} className={`${theme.card} border ${theme.border} rounded-xl p-5 relative overflow-hidden group`}>
...
const AdminDashboard = () => {
    ...
    const theme = user ? THEMES[user.theme || 'default'] : THEMES['default']; // Resolve theme
    ...
    return (
        <div className={`min-h-screen ${theme.bg} text-white flex transition-colors duration-300`}>
            <div className={`w-64 ${theme.sidebar} border-r ${theme.border} p-6 flex flex-col transition-colors duration-300`}>
                <div className="mb-10 flex flex-col items-start gap-3">{user.logoUrl ? <img src={user.logoUrl} alt={user.shopName} className="h-16 w-auto object-contain mb-2" /> : <Wrench size={32} className="text-blue-400" />}<h1 className="font-bold text-xl tracking-tighter text-white">{user.shopName}</h1></div>
                <nav className="space-y-2 flex-1">
                    <button onClick={() => setActiveTab('home')} className={`w-full text-left px-4 py-3 rounded-xl flex gap-3 ${activeTab === 'home' ? theme.activeTab : 'hover:bg-white/5'} transition-colors`}><LayoutDashboard size={20} /> Ο Στόλος</button>
                    <button onClick={() => setActiveTab('erp')} className={`w-full text-left px-4 py-3 rounded-xl flex gap-3 ${activeTab === 'erp' ? theme.activeTab : 'hover:bg-white/5'} transition-colors`}><Wallet size={20} /> Mini ERP</button>
                    <button onClick={() => setActiveTab('users')} className={`w-full text-left px-4 py-3 rounded-xl flex gap-3 ${activeTab === 'users' ? theme.activeTab : 'hover:bg-white/5'} transition-colors`}><Users size={20} /> Προσωπικό</button>
                    <button onClick={() => setActiveTab('settings')} className={`w-full text-left px-4 py-3 rounded-xl flex gap-3 ${activeTab === 'settings' ? theme.activeTab : 'hover:bg-white/5'} transition-colors`}><Settings size={20} /> Ρυθμίσεις</button>

                    {/* SUPER ADMIN BUTTON */}
                    {user.role === 'superadmin' && (
                        <button onClick={() => setActiveTab('superadmin')} className={`w-full text-left px-4 py-3 rounded-xl flex gap-3 mt-10 border border-purple-500/30 ${activeTab === 'superadmin' ? 'bg-purple-600/20 text-purple-300' : 'text-purple-400 hover:bg-purple-900/20'} transition-colors`}>
                            <LayoutDashboard size={20} /> Διαχείριση SaaS
                        </button>
                    )}
                </nav>
                <button onClick={() => setUser(null)} className="flex gap-2 items-center text-red-400 mt-6 hover:text-white"><LogOut size={18} /> Έξοδος</button>
            </div>
            <div className="flex-1 p-8 overflow-y-auto h-screen">
                ...
                {activeTab === 'superadmin' && user.role === 'superadmin' && <SuperAdminTab theme={theme} />}
            </div>
        </div>
    );
};
